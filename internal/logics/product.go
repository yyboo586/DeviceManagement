...
func (l *product) List(ctx context.Context, req *v1.ListProductReq) (out []*model.Product, total int, currentPage int, err error) {
	...
	m := dao.Product.Ctx(ctx).Where(dao.Product.Columns().OrgID, req.OrgID)
	// 分类ID过滤
	if req.CategoryID > 0 {
