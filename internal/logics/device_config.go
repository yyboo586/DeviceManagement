...
func (l *deviceConfigLogic) Add(ctx context.Context, in *v1.AddDeviceConfigReq) (err error) {
	for _, config := range in.List {
		...
		insertData := g.Map{
			dao.DeviceConfig.Columns().OrgID: in.OrgID,
...
func (l *deviceConfigLogic) Get(ctx context.Context, in *v1.GetDeviceConfigListReq) (out []*model.DeviceConfig, err error) {
	typ := model.GetDeviceConfigType(in.Type)
	if typ == model.DeviceConfigTypeUnknown {
	...
	err = dao.DeviceConfig.Ctx(ctx).Where(dao.DeviceConfig.Columns().OrgID, in.OrgID).Where(dao.DeviceConfig.Columns().Type, typ).Scan(&entities)
	if err != nil {
